...
sed -i 's,CONFIG_LOCALVERSION="-Hybridmax",CONFIG_LOCALVERSION="'$LOCALVERSION'",' .config
...
if [ -e $BK/$TARGET/ramdisk.* ]; then
	rm -rf $BK/$TARGET/ramdisk.*
fi;
...
if [ $USER != "root" ]; then
	make CONFIG_DEBUG_SECTION_MISMATCH=y -j$NUMBEROFCPUS Image ARCH=arm64
else
	make -j$NUMBEROFCPUS Image ARCH=arm64
fi;
...
# make modules
make -j$NUMBEROFCPUS modules ARCH=arm64  || exit 1;
...
zip -r SM-$TARGET-$DEVELOPER-kernel-${VERSION}-`date +[%d-%m-%y]`.zip .
#tar -H ustar -c ${KERNELDIR}/output/$TARGET/boot.img > $HYBRIDVER.tar
#md5sum -t $HYBRIDVER.tar >> $HYBRIDVER.tar
#mv $HYBRIDVER.tar $HYBRIDVER.tar.md5
